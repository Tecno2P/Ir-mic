...
void SdManager::begin() {
    // C-04 FIX: create the global VSPI bus mutex before any SPI transaction.
    // All modules that share VSPI must take this mutex before transacting.
    if (!g_spi_vspi_mutex) {
...
bool SdManager::_mount() {
    // C-04 FIX: take VSPI mutex before any SD SPI transaction.
    // NFC/NRF24/SubGHz (if on VSPI) and hw_poll task access the bus concurrently.
...
bool SdManager::triggerOtaFromSD(const String& target) {
    ...
    while (f.available() && ok) {
        ...
        otaMgr.handleUploadChunk(target, otaBuf, n, offset, fileSize, isFinal);
