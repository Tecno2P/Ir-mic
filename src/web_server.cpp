...
void WebUI::broadcastOtaProgress(size_t done, size_t total) {
    // FIX: OTA progress fires ~100× per second during flash write.
    // Static char buffer avoids 100+ String heap allocs per OTA.
    static char buf[96];
    uint8_t pct = (total > 0) ? (uint8_t)((done * 100UL) / total) : 0;
    int n = snprintf(buf, sizeof(buf),
        "{\"event\":\"ota_progress\",\"done\":%u,\"total\":%u,\"percent\":%u}",
        (unsigned)done, (unsigned)total, (unsigned)pct);
    if (n > 0 && _ws.count() > 0) _ws.textAll(buf, (size_t)n);
}

void WebUI::broadcastOtaResult(bool success, const String& message) {
    char buf[192];
    int n = snprintf(buf, sizeof(buf),
        "{\"event\":\"ota_result\",\"success\":%s,\"message\":\"%s\"}",
